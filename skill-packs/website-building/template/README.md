...
`src/themes/<主題>/` 各自擁有完整的版面、導覽、頁尾、首頁、文章列表與文章頁，以及自己的 `theme.css`。每個主題依一份公開設計指引實作，不是同一版面換顏色。

| id | 名稱 | 調性 | 設計指引來源（open-design，Apache-2.0） |
|---|---|---|---|
...
- 新增主題時要照一份可追溯的設計指引實作全部檔案，並在 `theme.json` 記錄來源與授權。
