...
- 頁面依需求選擇，部落格選配，六頁保留為範例；七技能依同一目標自動接續，Agent 先產稿與推薦設計、整體批次確認後自動建置及記錄，不把技術關卡交給人類。
...
- 預設 Cloudflare Workers 靜態資產與 workers.dev。已有網域可沿用，DNS 不在 Cloudflare 時先依已授權工具加入站點及調整 nameserver；只剩 Computer Use 才提供人工與較慢代做選項。OAuth、DNS、部署與公開收錄仍分開授權；不在包內保存 API Token。
