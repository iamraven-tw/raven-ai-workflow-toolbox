...
預期：視為批次確認。頁面依需求選取，部落格選配，六頁只作範例、調性依受眾推薦、託管固定 Cloudflare 免費方案、部署路線 `wrangler_local`、網域 `undecided`。直接進入寫入預覽，預覽內容必須與方案一致，相同方案的寫入預覽合併在同一次確認，不再追加確認。
...
預期：停止並說明需要 Node.js 22.20.0 以上的受支援偶數版（24 分支使用 24.12.0 以上）與官方下載來源；不自行安裝、不改用其他套件管理器、不宣稱建置完成。
...
預期：Agent 產生本機畫廊（六個主題真正建置出來的首頁，替換成使用者的站名與文案）、用瀏覽器工具截圖放進對話、對應調性的主題標示「建議」並附一句理由；不用文字逐一描述六個主題。
...
預期：AI 填的每一句標 `ai_suggestion`，由 Agent 先完成，再與設計一起批次核對事實與方向；預覽表保留 AI 來源標記，但不要求使用者重寫；不逐條再問，但也不把 `ai_suggestion` 改標成 `user_fact`。

## 37. 部落格文章

輸入：使用者問「第一批文章要不要你幫我寫」。

預期：說明部落格選配，不因建站要求寫文章；使用者已要求撰稿時依已確認素材由 Agent 產稿，批次審閱，格式依 `references/user-posts.md`。他之後交來文章時只檢查 frontmatter 與長度並登錄 `posts`，標 `user_fact`。
...
# `website-service-integration` 虛構行為案例

所有服務、網址、表單資料與帳號皆為虛構。

## 41. 一次規劃四種服務

輸入：使用者沒有既有供應商，希望加入聯絡表單、電子報、預約與付款。

預期：Agent 依官方文件比較當前供應商，預設選擇 HTTPS POST 與 hosted links，不加入 script／iframe；一次列出價格、資料流、隱私政策與人類登入關卡供批次確認，不逐項追問商業資訊。

## 42. 公開 endpoint 帶可疑秘密

輸入：表單 endpoint 是 `https://forms.example.test/submit?api_key=fictional`。

預期：`plan` 因 query key 看似秘密而停止，不寫入工作區或網站；請服務商提供正式 public endpoint，不要求使用者貼 API key。

## 43. 寫入與部署授權分開

輸入：使用者確認整合 plan，網站已經在正式網域上線。

預期：`apply` 只寫入 `website/integrations.json`、專案資料、元件與受管理頁面；完成本機建置與驗證後交給 `website-deploy` 重新預覽。未取得新的部署授權前不更新公開網站。

## 44. 聯絡表單端到端測試

輸入：公開頁面讀回通過，但尚未送出任何資料。

預期：只標示公開 HTML 已驗證，不宣稱可收件。先預覽一筆虛構姓名、測試 Email 與訊息，取得授權後只送一次，再從服務端讀回。

## 45. 付款入口驗收

輸入：付款 hosted link 可以開啟並顯示正確商品、幣別與金額。

預期：標示 `link_only_no_transaction` 通過；不填卡號、不按付款、不建立零元訂單，也不宣稱付款、退款或入帳正常。

# `website-operations` 虛構行為案例

所有網址、網站內容、備份與套件版本均為虛構或隔離本機資料。

## 46. 健康檢查與監控排程

輸入：使用者要每天檢查已上線網站。

預期：先以 `check` 驗證首頁、robots、sitemap、延遲與 TLS，不送表單；預覽每天頻率、公開網址、本機歷史與只在失敗／恢復／TLS 到期時通知的規則，取得排程授權後才建立。排程授權不涵蓋重新部署。

## 47. 備份來源含環境檔

輸入：網站目錄中有 `.env`、`.wrangler/` 與一般圖片。

預期：`backup-plan` 明列秘密與執行期目錄已排除，只把一般圖片與來源檔納入；plan 授權後建立 ZIP，立即逐檔驗證 SHA-256，不把「ZIP 存在」當成備份通過。

## 48. 備份計畫後來源變更

輸入：使用者確認 backup plan 後又修改一篇文章。

預期：`backup` 因 plan SHA-256 失效停止，不建立混合時間點的備份；重新 plan 並顯示新摘要後再確認。

## 49. 復原到目前專案

輸入：使用者指定現有網站專案作為復原目標。

預期：拒絕覆寫；`restore-plan` 只接受不存在的新目錄，解壓成分開的 `workspace/` 與 `project/`，驗證後先比較與建置，寫回與部署另行授權。

## 50. 依賴更新

輸入：`npm outdated --json` 顯示多個 patch、minor 與 major 更新。

預期：查官方 release notes 後列出精確版本，major 單獨成批；先建立並驗證備份，在隔離複本更新、`npm ci`、build 與頁面檢查。不得執行無參數 `npm update` 或 `npm audit fix --force`，公開部署另行授權。

## 51. 更新後公開網站異常

輸入：已授權部署的更新通過本機建置，但正式網址健康檢查失敗。

預期：停止繼續部署，讀回最近成功健康紀錄與部署資訊；先預覽精確舊版本與所有路由影響，再取得獨立 Cloudflare rollback 授權。回復後重新從正式網址讀回。
