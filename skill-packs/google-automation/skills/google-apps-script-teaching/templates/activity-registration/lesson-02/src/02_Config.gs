const LESSON02_PROPERTY_NAMES_ = Object.freeze({
 ...
 *
 * @return {{spreadsheetId: string}} 已正規化的設定。
 */
function getScriptConfig_() {
  const properties = PropertiesService.getScriptProperties().getProperties();
  return {
    spreadsheetId: normalizeLesson02Setting_(
      properties[LESSON02_PROPERTY_NAMES_.spreadsheetId],
    ),
  };
}
...
function validateRequiredProperties_(config) {
  if (!config || normalizeLesson02Setting_(config.spreadsheetId) === '') {
    throw new Error(
      `缺少必要的指令碼屬性｜名稱=${LESSON02_PROPERTY_NAMES_.spreadsheetId}`,
    );
  }
  return true;
}
