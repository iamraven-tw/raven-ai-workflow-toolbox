 ...
 * 取得目前專案的集中式設定。
 *
 * 實際值只存在 Apps Script 的指令碼屬性，不會寫入程式、
 * 紀錄檔(Log)或 Git。
 *
 * @return {{
...
function getScriptConfig_() {
  ...
  return {
    ...
    docTemplateId: normalizeLesson02Setting_(
      properties[COURSE_PROPERTY_NAMES_.docTemplateId],
    ),
    outputFolderId: normalizeLesson02Setting_(
      properties[COURSE_PROPERTY_NAMES_.outputFolderId],
    ),
    webhookToken: normalizeLesson02Setting_(
      properties[COURSE_PROPERTY_NAMES_.webhookToken],
    ),
    webAppUrl: normalizeLesson02Setting_(
      properties[COURSE_PROPERTY_NAMES_.webAppUrl],
    ),
  };
}
