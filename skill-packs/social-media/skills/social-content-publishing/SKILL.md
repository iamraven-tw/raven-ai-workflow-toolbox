...
description: "發布已確認的社群文案與媒體；管理自有 Facebook、Instagram、Threads、YouTube 內容的刪除，以及 Facebook 文字與 YouTube 標題／說明修改。先完整預覽與確認，再單次執行、獨立讀回及記錄結果。Substack 僅發布；留言、私訊與成效交對應技能。"
---
...
要求修改或刪除既有內容時，先讀 [內容管理契約](references/content-management.md)，不要套用新上傳的素材、排程或通知問答。Agent 先按需唯讀取得指定內容，提供一次完整變更預覽；取得明確的修改／刪除確認後沿用同一交易帳本。官方支援不等於本套件已實機驗收。
...
停止條件：缺核准、缺成品、目的地不明、格式或變體沒有明確路由、當前版本／額度不能確認、授權不足、驗證失敗、重複工作、鎖定衝突、未明寫入結果。先做已授權且有限的唯讀查明；不得藉「自動修復」重送或清除鎖。官方範例的固定上限可能過時；YouTube 查實際專案配額，Instagram／Threads 使用官方額度端點，Facebook Reels 精確 API 限制若無法刷新就走受控介面。原生排程只限文件明確列出的平台與格式；不替使用者建立本機／週期排程。Facebook 同 App 貼文文字修改改讀 [修改契約](references/publishing-contract.md#facebook-文字修改)，沿用一次確認及帳本，讀回成功記 updated。其他未列入 adapter 的編輯、刪除、直播、Stories、Notes 路徑先回報缺口，不套用一般貼文端點。
