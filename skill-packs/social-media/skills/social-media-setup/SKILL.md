...
Threads 回呼註冊受阻且使用者仍要求完成自有帳號 API 測試時，可依 [官方測試權杖匯入](references/threads-token-import.md) 使用後台用戶權杖產生器。這是獨立於上述 authorization-code callback 的匯入流程：沿用既有授權，先核對原生庫與相容匯入程式，再取得秘密；不需要一個已成功註冊的 callback。不得把匯入成功標成回呼驗收成功，也不得自行輸入或重放 OAuth code。只有本人同意與隱藏輸入交接需要人操作，其他驗證由 Agent 完成。
