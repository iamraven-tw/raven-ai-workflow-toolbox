...
description: "建立或調整社群媒體工作流的策略與平台整合。當使用者要開始社群經營、設定策略、選擇 YouTube、Instagram、Facebook、Threads 或 Substack、檢視完整功能權限、實際建立開發者 App／API／OAuth、驗證平台讀取、設定權杖定期檢查與刷新，或檢查目前設定時使用。優先 API → MCP → OpenCLI；Computer Use 僅在使用者特別要求且其他方式無法完成時作為最後手段；平台整合預設提出該平台已支援核心功能的完整權限，逐項說明並允許使用者在 OAuth 前刪減；需要維護的權杖預設建立排程並立即驗證；沒有明確決策時一次只問一個最重要的問題。"
---
...
- 社群媒體設定優先 API → MCP → OpenCLI；需要操作後台畫面時預設提供人工文字指引，Computer Use 依下方工具選擇規則作為例外。
- 沒有可用 API／MCP／OpenCLI 的 App 建立、API 啟用、權限、回呼與測試角色設定，預設由人類依指引完成；Agent 可依授權準備本機接收器、安全保存與正式 API 讀取驗收。
- API／OAuth 建立後，預設把憑證存入目前作業系統帳號的 macOS Keychain 或 Windows Credential Manager；使用者可在取得秘密前改選或拒絕。
- OAuth 權杖會到期、可刷新、可撤銷或受資料存取期限影響時，把生命週期維護納入同一次整合：說明平台差異、建立定期檢查／刷新排程並立即驗證。使用者明確拒絕排程時記錄中斷風險，不把整合標為完整完成。
- 將外部工作拆成 API／App、OAuth／登入、平台讀取與遠端寫入等獨立關卡。
- 先顯示完整設定預覽，取得明確確認後才寫入一般設定。
- 分層回報已完成、尚未驗證與必須由人完成的項目。

本技能不撰寫內容、不製作圖片、不發布、不回覆留言、不建立內容發布排程，也不執行成效分析。這些工作應交給對應技能；OAuth 憑證的定期檢查與刷新排程則屬於本技能的整合責任。
...
4. 讀取該模式的參考文件：
   ...
   - 實際建立或沿用 OAuth 權杖時，再讀 `references/token-lifecycle-maintenance.md`，把維護排程列入完整整合預覽與驗收。
5. 先依工具選擇規則完成可用的已授權操作，剩餘後台步驟依下方「實際整合執行」交付人工申請清單；只要求規劃時停在指引。使用者完成平台設定並提供必要資料後，Agent 才接續已授權的本機憑證保存、OAuth 接收與正式 API 驗收。
6. 形成候選設定後，依 `references/configuration-contract.md` 驗證並預覽。
7. 使用者確認預覽後才寫入；重新讀回檔案並比對雜湊。
8. 依 `references/verification-levels.md` 分開回報本機設定、API／App、登入／OAuth、平台讀取、憑證維護與遠端寫入，不猜測較高層已完成。

## 平台整合原則

每個「平台 × 功能」仍須獨立驗證，但首次選取平台時預設採完整管理模式，授權預覽涵蓋該平台所有已支援核心功能。設定與讀取都依 API → MCP → OpenCLI 順序選擇可用且已授權的介面；需要後台畫面時預設提供人工指引，Computer Use 僅依下節例外執行。不得把瀏覽器內部請求或第三方套件冒充官方 API。
...
## 工具選擇與 Computer Use 例外

1. **API**：優先使用正式、已查證且具備必要授權的 API；既有連線可用就沿用。
2. **MCP**：API 無可用執行路徑時，使用具備對應能力與授權的 MCP 工具。
3. **OpenCLI**：前兩者無法完成時，使用已安裝、已驗證的 OpenCLI 命令／adapter；不能把它說成官方 API。
4. **人工文字指引**：以上皆無可用路徑，或仍缺平台後台設定時，一次提供完整操作步驟與必要資料清單。
5. **Computer Use 最後手段**：只有使用者特別要求 Computer Use，且上述優先工具無法完成時才執行。先簡述不可用原因、速度較慢對體驗的影響及具體操作範圍；同範圍已明確要求就沿用，不重複索取許可。一般的「全部做完」「繼續」或概括授權不等於特別要求 Computer Use。不得因一個逾時就自動切換，也不要主動反覆勸使用者採用。

以實際動作判定介面：透過 MCP 包裝的畫面點擊仍屬 Computer Use，不因工具名稱是 MCP 就繞過例外條件。OpenCLI 的既有命令可優先使用；沒有適用命令時不得假造能力或把任意畫面點擊換名為 OpenCLI。更換工具不擴張帳號、權限、遠端寫入或秘密存取授權；結果不明的寫入不得換介面重送。

下列平台文件中的人工步驟是缺少優先工具路徑時的預設指引；符合本節條件才可用 Computer Use 處理必要畫面。憑證仍依安全輸入契約，登入／安全驗證及平台要求本人同意的步驟不因此改由 Agent 代按。
...
8. 只有讀回結果與使用者指定目標一致才標示平台讀取已驗證。依 `references/token-lifecycle-maintenance.md` 建立或更新同一工作區、連線與平台的維護排程，立即執行一次並讀回非敏感結果；排程不存在或首次執行失敗時，整合只能標示 `partial`。遠端發布、回覆或內容發布排程仍保持未測試，除非另有明確授權。

外部變更預覽至少列出：平台與功能、將建立或調整的 App、目前官方 use case／產品名稱、完整核心權限及其用途、相依權限與額外影響、可選延伸權限、使用者刪減項目、OAuth callback 類型、偵測到的原生憑證庫、要保存的憑證名稱、未來 Agent 可取用的範圍、權杖生命週期、維護排程頻率、執行環境、正常靜默與異常通知方式、預期人工關卡、驗證請求，以及本輪明確排除的遠端操作。預覽要明示使用者可在取得秘密前改選或拒絕持久保存與維護排程；未提出不同選擇時採原生憑證庫與每日維護檢查。預覽結尾必須直接告知使用者「如果有任何權限不想開放，請現在告訴我」；後台實際名稱或權限與預覽不同時停止並重新確認，不自行改選相近項目。
...
透過優先工具、完整指引與既有授權沿用，減少等待、重複問答與重做。
...
- 設定預覽或寫入後的工作區相對路徑。
- 各層狀態：本機設定、本機憑證儲存、API／App、使用者登入／OAuth、平台讀取、`credential_maintenance`、遠端寫入；維護層另列排程頻率、最後檢查、是否刷新與下一次需要本人處理的條件。
...
一般檔案不得寫入技能目錄、公開 Toolbox、策略洞察檔、平台內容或內容發布排程。未取得外部變更預覽的明確授權時，不得修改任何外部服務；Agent 取得授權後可依工具選擇規則完成預覽內的 App／API、連線設定、OAuth 協定交換、讀取驗收與憑證維護排程，不得發布、回覆或建立內容發布排程。平台私人識別碼與憑證值只能寫入使用者選定的秘密儲存；上述參照檔只記錄可重新定位秘密的非敏感資訊。使用者未另行指定時，外部變更預覽確認同時涵蓋原生憑證庫寫入與必要的定期維護；保存憑證不代表已授權發布，也不取代後續發布或管理技能的遠端確認。若使用者另行要求更新策略洞察，交給未來的 `social-performance-analysis`，並保留其預覽與再次確認關卡。
...
- 權杖需要持續維護，但執行環境沒有可用排程器、無法在同一作業系統使用者下取用原生憑證庫，或首次維護執行失敗；保留整合成果並標示 `credential_maintenance: not_configured` 或 `attention_required`。
- 候選設定含秘密、私人識別碼或憑證路徑。
...
公開候選版以虛構工作區驗證：缺少設定、完整核心權限預設、使用者刪減、延伸權限選擇、Meta 三平台合併詢問但分開驗證、一次一問、兩種模式、只讀選取的平台文件、秘密欄位拒絕、預覽前不寫入、錯誤雜湊拒絕、確認後原子寫入、既有變動停止、讀回雜湊、原生憑證庫參照不含秘密、隱藏互動輸入、單筆取代／刪除關卡、權杖維護只依既有 Runtime 安全刷新且不重送不明操作，以及技能錯誤的一次小修正與停止邊界。虛構 backend 測試不證明 macOS、Windows、排程產品或任何平台連線已實測。
