...
依 [主技能的最小化人類操作](../SKILL.md#最小化人類操作)，在最早可完成盤點時集中預覽一次；已核准項目沿用，不按平台或流程階段重問。HTTPS 缺口應在此預覽列出具體安裝、憑證信任與 hosts／部署方案，不延到取得 Secret 前才首次提出。登入前無法確認的資訊標示待查，後續只確認真正新增的影響。

點擊建立 App 前顯示並確認以下內容；同一份已確認預覽不再重問：
...
Agent 一次提供完整步驟，不要求逐頁回覆，不自動啟動 Computer Use。
