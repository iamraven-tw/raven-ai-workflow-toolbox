...
7. Agent 先檢查可用受控瀏覽器、callback 與秘密儲存。使用者未指定時，依 `local-credential-storage.md` 預設採 macOS Keychain 或 Windows Credential Manager，並把 backend 與憑證名稱列入外部變更預覽；任一必要條件缺少時停止於取得 Secret 或 Token 之前。
