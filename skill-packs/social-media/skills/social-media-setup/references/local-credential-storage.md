...
- 將真正需要的人工作業和一般許可分開：密碼重新驗證／2FA 是登入關卡，不是請使用者再複製一次 Secret；完成後由人類取得缺少的金鑰並以隱藏輸入提供，AI 接續檢查保存及驗收。新的敏感存取仍遵守所用工具的當下確認規則，既有授權不重問。

## 人工提供憑證

Agent 列出本輪缺少的憑證名稱、平台／App 來源與用途，提供取得位置的文字步驟。人類自行操作後台並在本機隱藏輸入提供；不得貼進聊天。既有 `credential_browser.py` 僅保留舊流程相容性，本技能不啟動它，也不使用 Computer Use 或瀏覽器自動讀取、搬運、保存秘密。
