...
下表只供使用者明確提出延伸需求時查閱，不列入自用內容、成效、留言與私訊的預設問答。只有已選功能確實需要的類別才加入 OAuth；實際 permission 名稱與相依項仍須在執行當天重新查證。此為工作流範圍調整，不表示下表所有平台權限已重新驗證。
...
不要預設要求使用者電子郵件、廣告、付款、企業資產、商品或訪客個人化資料。官方後台出現相鄰權限不代表需要申請；與已選功能無關就略過，必要相依項才重新查證並解釋影響。
...
- [建立 Messenger 使用案例](https://developers.facebook.com/docs/development/create-an-app/messenger-use-case/)
