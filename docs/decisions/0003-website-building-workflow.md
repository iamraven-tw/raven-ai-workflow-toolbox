...
頁面依實際需求選擇，部落格選配；原六頁是範例而非強制清單。Agent 先完成文案與設計推薦，人類整體批次確認事實與方向；建置、檢查、截圖、修正及技術狀態自動執行。七技能內部接續不要求重下指令。登入、本人驗證、首次公開部署、網域購買與付費保留；外部動作可在明確範圍內合併授權，未授權者不自動執行。下列歷史決策與此修訂不同時，以本節及套件人類接觸點清單為準。
