...
1. 一人公司的長期方向與限制，經使用者確認後寫入 `sources/strategy/`。
...
| 技能 | 用途 |
|---|---|
| `my-real-second-brain-setup` | 引導 AI Agent 安裝、註冊及驗證固定版本的預設後端。 |
...
| 外部能力 | 用途 | 本專案的處理方式 |
|---|---|---|
| `notebooklm` 通用技能與 `notebooklm-py` | 建立 Notebook、加入來源、列出來源、查詢與取得引用 | AI Agent 依鎖定版本自動安裝上游套件與技能；本倉庫不重製原始碼。 |
| Graphify | 建立、查詢與視覺化本地知識圖譜 | AI Agent 依鎖定版本自動安裝上游套件與技能；本專案定義使用及 HTML 交付規則。 |

Graphify 與 Notebook 是目前的預設後端，不是系統本身。詳細邊界與替換契約請見 `docs/providers.md` 及 `docs/integrations/`。

## 公開核心與實際案例

本 repository 保存通用技能、安裝契約與空白工作區範本；真實使用案例則保存自己的專案規則、筆記、索引與研究資料。實際案例驗證出的修正，只有在去識別化並確認具有通用價值後才回饋公開核心。完整規則見 `docs/development-workflow.md`。
...
3. 取得同意後，把五個自有技能安裝到使用者範圍。
4. 確認第二大腦工作區，合併 `template/` 並保留所有既有檔案。
5. 安裝預設後端與該工作區範圍的上游技能。
