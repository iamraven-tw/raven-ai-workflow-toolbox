...
平台 API／OAuth 機密的目標位置為目前作業系統帳號的 macOS Keychain 或 Windows Credential Manager；依共用契約，輸入與儲存必須由人類在安全入口操作，Agent 只準備入口並驗證去敏感結果。現有社群程式的自動儲存路徑待下一階段改造。一般設定只保存整合意圖與不含秘密值的參照；保存憑證不等於授權發布或其他遠端寫入。
...
每個步驟先檢查可行的 API、連接器、CLI 或 OpenCLI。確認只剩 Computer Use 時，Agent 提供人工操作指引，詢問是否需要代做並提醒速度可能較慢；取得該範圍同意後才操作。機密輸入與儲存、OAuth 同意及平台要求本人完成的步驟，仍由人類親自操作，不能因同意代做而跳過。完整分工、授權沿用及接續方式見[共用人機分工契約](docs/human-agent-contract.md)。
